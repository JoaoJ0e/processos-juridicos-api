...
public class Telefone {
    ...
    public Telefone(String numero) {
        if (numero == null) throw new IllegalArgumentException("Telefone não pode ser nulo.");

        String numeroLimpo = limpar(numero);

        if (!numeroLimpo.matches("^\\d{10,11}$")) throw new IllegalArgumentException("Telefone inválido.");
